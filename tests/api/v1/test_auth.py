...
@pytest.mark.asyncio
async def test_login_wrong_password(client: AsyncClient, override_get_db, mock_redis_manager, mocker):
...
@pytest.mark.asyncio
async def test_login_user_not_found(client: AsyncClient, mock_db_session, override_get_db, mock_redis_manager, mocker):
...
@pytest.mark.asyncio
async def test_login_inactive_user(client: AsyncClient, mock_db_session, override_get_db, mock_redis_manager, mocker):
...
@pytest.mark.asyncio
async def test_login_locked_user(client: AsyncClient, override_get_db, mock_redis_manager, mocker):
    """Test login when user is locked"""
    # Mock security.is_user_locked to return True
    # We can mock the function directly
    mocker.patch("app.core.security.is_user_locked", new_callable=AsyncMock, return_value=True)
