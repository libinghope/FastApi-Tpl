...
async def list_users(
    ...
    Get user list
    """
    stmt = select(SysUser, SysDept.name.label("dept_name")).outerjoin(
        SysDept, SysUser.dept_id == SysDept.id
    )
    ...
    users_with_dept = (
        result.all()
    )  # list of (SysUser, dept_name) or just (SysUser,) if outer join selected specific cols
    ...
    return ResponseSchema(result=PageSchema(list=users, total=total))
...
async def add_user(
    ...
    if await db.scalar(stmt):
        return ResponseSchema(
            code=ErrorCode.USER_ALREADY_EXISTS, message="Username already exists"
        )
...
async def update_user(
    ...
    if not form.id:
        return ResponseSchema(
            code=ErrorCode.INVALID_ARGUMENT, message="User ID is required"
        )
    ...
    if form.role_ids is not None:
        # Delete old roles
        await db.execute(
            delete(SysUserRoleRef).where(SysUserRoleRef.user_id == user.id)
        )
        # Add new
        for rid in form.role_ids:
...
async def delete_users(
    ...
    if current_user.id in form.uid_arr:
        return ResponseSchema(
            code=ErrorCode.INVALID_ARGUMENT, message="Cannot delete yourself"
        )
    ...
    await db.execute(
        delete(SysUserRoleRef).where(SysUserRoleRef.user_id.in_(form.uid_arr))
    )
...
async def reset_password(
    ...
    if form.password != form.password2:
        return ResponseSchema(
            code=ErrorCode.PASSWORD_MISMATCH, message="Passwords do not match"
        )
    ...
    return ResponseSchema(result={"relogin": relogin})
...
async def change_active_status(
    ...
    if form.user_uid == current_user.id:
        return ResponseSchema(
            code=ErrorCode.INVALID_ARGUMENT, message="Cannot change your own status"
        )
...
async def get_user_options(
    ...
    return ResponseSchema(
        result={"result": [{"label": u.username, "value": u.id} for u in users]}
    )
