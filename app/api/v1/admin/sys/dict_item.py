...
from app.schemas.sys.dict import (
    DictItemCreate,
    DictItemUpdate,
    DictItemResponse,
    DeleteObjsForm,
...
async def get_dict_items(
    ...
    return ResponseSchema(
        result=[DictItemResponse.model_validate(item) for item in items]
    )
...
async def update_dict_item(
    ...
    if not item:
        return ResponseSchema(
            code=ErrorCode.DICT_ITEM_NOT_FOUND, message="Dict item not found"
        )
