...
async def create_sys_dept(db: AsyncSession):
    depts = [
        SysDept(
            id=1, parent_id=0, tree_path="0", name="有来技术", code="YOULAI", sort=1
        ),
        SysDept(
            id=2, parent_id=1, tree_path="0,1", name="研发部门", code="DEV", sort=1
        ),
        SysDept(
            id=3, parent_id=1, tree_path="0,1", name="测试部门", code="TEST", sort=2
        ),
    ]
    db.add_all(depts)
